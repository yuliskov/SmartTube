...
test('places the TV button after Share and reuses the saved pairing', async () => {
  ...
  let svgReady = false;
  const timers = [];
  const svg = { replaceChildren(path) { this.path = path; } };
  const shareButton = {
    cloneNode() {
      button = {
        style: {},
        ...
        querySelector(selector) {
          if (selector === 'svg') return svgReady ? svg : null;
  ...
  const context = {
    document: {
      ...
      createElementNS() { return { setAttribute(name, value) { this[name] = value; } }; },
  ...
  visibleRows = [row];
  timers.shift()();
  assert.equal(button.parentElement, undefined);
  svgReady = true;
  timers.shift()();
  ...
  assert.match(svg.path.d, /^M3 4h18/);
